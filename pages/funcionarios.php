<?php
/**
 * Página de Gestão de Funcionários - Versão com Sistema de Personificação
...
// =====================================================
// SISTEMA DE PERSONIFICAÇÃO/SIMULAÇÃO
// =====================================================

// Processar volta da simulação de forma mais robusta
if (isset($_POST['voltar_simulacao']) || isset($_GET['voltar_simulacao'])) {
    if ($auth->estaSimulando()) {
        ...
        if (isset($_POST['voltar_simulacao']) && !empty($_SERVER['HTTP_X_REQUESTED_WITH'])) {
            header('Content-Type: application/json');
            echo json_encode(['status' => 'success', 'message' => 'Simulação encerrada']);
            exit;
        }
        
        // Redireciona com mensagem
        header('Location: ' . $_SERVER['PHP_SELF'] . '?msg=simulacao_encerrada');
        exit;
    }
}

// Processar início da simulação
if (isset($_POST['action']) && $_POST['action'] === 'simular' && isset($_POST['funcionario_id'])) {
    if ($auth->isAdmin() || $auth->isDiretor()) {
        $funcionario_id = (int)$_POST['funcionario_id'];
        
        // Não permitir simular a si mesmo
        if ($funcionario_id == $usuarioLogado['id']) {
            $_SESSION['erro'] = 'Você não pode simular sua própria conta!';
        ...
        if ($resultado['success']) {
            $_SESSION['sucesso'] = 'Simulação iniciada com sucesso!';
        ...
        $_SESSION['erro'] = 'Você não tem permissão para simular outros usuários!';
        header('Location: ' . $_SERVER['PHP_SELF']);
        exit;
    }
}

// Registrar ações durante simulação (para auditoria)
if ($auth->estaSimulando()) {
    $usuarioAtual = $auth->getUsuarioAtual();
    error_log(sprintf(
        "[SIMULAÇÃO] %s está simulando %s - Página: %s",
        $usuarioAtual['usuario_real']['nome'] ?? 'Admin',
        $usuarioAtual['nome'],
        $_SERVER['PHP_SELF']
    ));
}
...
$mensagemFeedback = '';
if (isset($_GET['msg'])) {
    if ($_GET['msg'] === 'simulacao_encerrada') {
        $mensagemFeedback = '<div class="alert alert-info alert-dismissible fade show" role="alert">
            <i class="fas fa-info-circle"></i> Simulação encerrada com sucesso! Você voltou para sua conta original.
...
if (isset($_SESSION['erro'])) {
    $mensagemFeedback = '<div class="alert alert-danger alert-dismissible fade show" role="alert">
...
if (isset($_SESSION['sucesso'])) {
    $mensagemFeedback = '<div class="alert alert-success alert-dismissible fade show" role="alert">
...
$departamentosEstrategicos = [1, 2, 9, 10, 15];
...
// Define permissões baseadas no contexto
if ($auth->isAdmin() || $estaNoDepartamentoEstrategico) {
    ...
    $podeEditar = true;
    $podeCriar = true;
    $podeSimular = ($cargoUsuario === 'Diretor'); // Apenas diretores podem simular em seu departamento
} else {
...
<head>
    ...
    <style>
        ...
        /* Ajusta o body quando tem banner de simulação */
        body.simulacao-ativa {
            padding-top: calc(var(--header-height, 70px) + 64px) !important;
        }

        /* Indicador no avatar do usuário quando simulando */
...
<body <?php echo $auth->estaSimulando() ? 'class="simulacao-ativa"' : ''; ?>>
    <!-- INDICADOR DE SIMULAÇÃO -->
    <?php if ($auth->estaSimulando()): ?>
        <?php 
            ...
            <div class="simulacao-content">
                ...
                <div class="simulacao-info">
                    ...
                    <span class="simulacao-detalhes">
                        ...
                        (<?php echo htmlspecialchars($usuarioSimulado['cargo'] ?? 'Sem cargo'); ?>)
                    </span>
                </div>
    ...
    <script>
        ...
        const podeSimular = <?php echo json_encode($podeSimular); ?>;
        ...
        const idSimulado = <?php echo json_encode($auth->estaSimulando() ? $auth->getUsuarioAtual()['id'] : null); ?>;
        ...
        function renderizarTabela() {
            ...
            todosFuncionarios.forEach((funcionario, index) => {
                ...
                // Adiciona classe se está simulando este funcionário
                if (estaSimulando && funcionario.id == idSimulado) {
                    row.classList.add('simulando-atual');
                }
                ...
                let rowHtml = `
                    ...
                    <td>
                        ...
                        ${estaSimulando && funcionario.id == idSimulado ? '<br><small class="text-danger"><i class="fas fa-user-secret"></i> Simulando agora</small>' : ''}
                        <br>
                ...
                // Botão de simulação com validações melhoradas
                if (podeSimular && funcionario.id != usuarioLogadoId && !estaSimulando) {
                    rowHtml += `
                            <button class="btn-icon simulate" onclick="event.stopPropagation(); simularFuncionario(${funcionario.id}, '${funcionario.nome.replace(/'/g, "\\'")}', '${funcionario.cargo || 'Sem cargo'}')" title="Simular">
                                <i class="fas fa-user-secret"></i>
                            </button>
                    `;
                } else if (estaSimulando && funcionario.id == idSimulado) {
                    rowHtml += `
                            <button class="btn-icon simulate active" onclick="event.stopPropagation(); alert('Você já está simulando este funcionário!')" title="Simulando agora">
        ...
        // Função de simulação melhorada
        function simularFuncionario(id, nome, cargo) {
            // Verifica se já está simulando
            if (estaSimulando) {
                alert('⚠️ Você já está em modo de simulação!\n\nSaia da simulação atual antes de iniciar uma nova.');
                return;
            }
...
🎭 MODO SIMULAÇÃO
━━━━━━━━━━━━━━━━━━━━━━━━
Você está prestes a simular:
• Nome: ${nome}
• Cargo: ${cargo}

⚠️ ATENÇÃO:
• Você assumirá todas as permissões deste usuário
• Suas ações serão registradas em nome dele
• Um banner vermelho indicará o modo simulação
