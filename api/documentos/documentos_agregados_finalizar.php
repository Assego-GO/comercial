 ...
 * 
 * Esta API pode ser usada para um fluxo em 2 etapas:
 * 1. Assinar (pendente -> assinado)
 * 2. Finalizar (assinado -> ativo)
 * 
 * Ou como alternativa à API de assinar para ativar diretamente
 */
...
try {
    ...
    try {
        ...
        if ($agregado['situacao'] === 'ativo') {
            $response = [
                'status' => 'success',
                'message' => 'Agregado já está ativo',
                'data' => [
        ...
        $observacaoCompleta = "[FINALIZAÇÃO " . date('d/m/Y H:i') . " - Func.ID: $funcionarioId] " . $novaObservacao;
